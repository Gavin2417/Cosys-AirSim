...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            sum_grid   = np.histogram2d(ground_x_vals, ground_y_vals, bins=[x_edges, y_edges],
                            weights=ground_z_vals)[0]
            count_grid = np.histogram2d(ground_x_vals, ground_y_vals, bins=[x_edges, y_edges])[0]
            Z_ground   = sum_grid / np.maximum(count_grid, 1)   # avoid divide‐by‐zero
            Z_ground[count_grid == 0] = np.nan

            # Calculate risk grids.
            non_nan_indices = np.argwhere(~np.isnan(Z_ground))
            step_risk_grid, slope_risk_grid = calculate_combined_risks(
                Z_ground, non_nan_indices, max_height_diff=0.025, max_slope_degrees=20.0, radius=0.5
            )
            ...
            threshold = 0.33 * max_risk
            mask = total_risk_grid > threshold
            total_risk_grid[mask] = np.exp(total_risk_grid[mask])
   
            
            total_risk_grid = interpolate_in_radius(total_risk_grid, 1)
            ...
            if path:
                ...
                if current_target_index < len(smoothed_path):
                    ...
                    throttle = 0 if abs(steering) > 0.60 else throttle_value 
