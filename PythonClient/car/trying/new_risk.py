...
masked_risk_grid = ma.masked_invalid(obstacle_grid)
...
fig.colorbar(c1, ax=axes[0], label='Slope Risk (0-1)')
axes[0].set_xlabel('X')
axes[0].set_ylabel('Y')
...
fig.colorbar(c2, ax=axes[1], label='Step Risk (0-1)')
axes[1].set_xlabel('X')
axes[1].set_ylabel('Y')
axes[1].set_title('Step Risk')

# Obstacle Grid Plot
c3 = axes[2].pcolormesh(X, Y, masked_obstacle_grid.T, shading='auto', cmap=cmap, vmin=0, vmax=1)
fig.colorbar(c3, ax=axes[2], label='Obstacle (0=No, 1=Yes)')
axes[2].set_xlabel('X')
axes[2].set_ylabel('Y')
axes[2].set_title('Obstacle Grid')
