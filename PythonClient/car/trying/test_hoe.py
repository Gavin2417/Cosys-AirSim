   ...
   # Map setup:
    ...
    try:
        while True:
            ...
            # annulus fill right after CVaR: set NaN cells at 1.7–3.5 m to 50
            ...
            if distance_from_vehicle.shape != risk_grid.shape:
                if distance_from_vehicle.T.shape == risk_grid.shape:
                    ...
                    distance_from_vehicle = np.sqrt((X - vehicle_x)**2 + (Y - vehicle_y)**2)
            annulus_mask = (distance_from_vehicle >= 1.7) & (distance_from_vehicle <= 3.5)
            nan_mask = np.isnan(risk_grid)
            risk_grid[nan_mask & annulus_mask] = 50

            # then mask far cells and fill remaining NaNs to 25
            
            risk_grid[np.isnan(risk_grid)] = 25
            ...
            if STEP_config['visualize']:
                ...
                # two circles centered at current vehicle position
                inner_r = 1.7
                outer_r = 3.5
                ax.add_patch(Circle((vehicle_y, vehicle_x), inner_r, fill=False, edgecolor='cyan', linewidth=1.5, linestyle='--', zorder=5))
                ax.add_patch(Circle((vehicle_y, vehicle_x), outer_r, fill=False, edgecolor='magenta', linewidth=1.5, linestyle='-', zorder=5))
