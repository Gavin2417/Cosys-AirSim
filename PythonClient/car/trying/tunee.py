...
def smooth_path(path, window_size=5):
    ...
    if window_size % 2 == 0:
        window_size += 1
    hw = window_size // 2
    return np.array([path[max(0,i-hw):min(n,i+hw+1)].mean(axis=0) for i in range(n)])
...
def run_simulation(steering_kp, steering_ki, steering_kd,
                   forward_kp, forward_ki, forward_kd,
                   max_iterations=200):
    """
    Runs your main loop for up to max_iterations, using the supplied PID gains.
    Returns the sum of minimum distances from vehicle to the manual path each step,
    plus a penalty if the destination isn't reached.
    """
    # --- Setup AirSim client, ground segmenter, etc. ---
    lidar_test = lidarTest('gpulidar1', 'CPHusky')
    lidar_test.client.enableApiControl(True, 'CPHusky')
    # ground segmentation
    BASE_DIR = os.path.dirname(__file__)
    config_path = os.path.join(BASE_DIR, "../assets/config.toml")
    groundseg = ground_seg(config_path) if os.path.exists(config_path) else ground_seg()

    grid_map_ground = GridMap(resolution=0.1)
    grid_map_obstacle = GridMap(resolution=0.1)
    # --- Initialize PIDs ---
    steering_pid = PIDController(steering_kp, steering_ki, steering_kd)
    forward_pid  = PIDController(forward_kp, forward_ki, forward_kd)
    ...
    for _ in range(max_iterations):
        ...
        # get_time_1 = time.time()- last_control
        # print(f"Time taken for lidar processing: {get_time_1:.2f} seconds")
        # last_control = time.time()
        ...
        # get_time_2 = time.time() - last_control
        # print(f"Time taken for grid building: {get_time_2:.2f} seconds")
        # last_control = time.time()
        ...
        # get_time_3 = time.time() - last_control
        # print(f"Time taken for risk calculation: {get_time_3:.2f} seconds")
        # last_control = time.time()
        ...
        total_risk_grid = fade_with_distance_transform(total_risk_grid,
                                                        ...
                                                        fade_scale=4.0,
                                                        sigma=3.0)
        # get_time_4 = time.time() - last_control
        # print(f"Time taken for fading: {get_time_4:.2f} seconds")
        # last_control = time.time()
        ...
        # get_time_6 = time.time() - last_control
        # print(f"Time taken for masking: {get_time_6:.2f} seconds")
        # last_control = time.time()
        ...
        if path:
            ...
            if current_target_index < len(smoothed_path):
                ...
                steering = steering_pid.compute(heading_error,dt)
                ...
                throttle_value = forward_pid.compute(forward_error,dt)
                steering_scale = max(0, 1 - abs(steering))  # scales from 1 to 0 as steering increases
                throttle = throttle_value * steering_scale
                lidar_test.client.setCarControls(
                    airsim.CarControls(throttle=throttle, steering=steering), lidar_test.vehicleName
                )
            vehicle_pos = np.array([vehicle_x, vehicle_y])
            path_distances = np.linalg.norm(smoothed_path - vehicle_pos, axis=1)
        ...
        if collision_info.has_collided:
            print("Collision")
            error_list.append(1000)
        # get_time_9 = time.time() - last_control
        # print(f"Time taken for control: {get_time_9:.2f} seconds")
        # last_control = time.time()

        distance_last = np.linalg.norm(destination_point - np.array([vehicle_x, vehicle_y]))
        if distance_last < 0.75 or sum(error_list) > 5000:
            print("Reached destination or error too high")
            lidar_test.client.setCarControls(airsim.CarControls(throttle=0, steering=0), lidar_test.vehicleName)
            break
        # stop if reached final point
    

    # clean up
    lidar_test.client.reset()
    lidar_test.client.simSetObjectPose(lidar_test.vehicleName, airsim.Pose(airsim.Vector3r(-5, -1, 0), airsim.Quaternionr(0, 0, 0, 1)), True)
    ...
    final_pos = np.array([vehicle_x, vehicle_y])
    if np.linalg.norm(final_pos - destination_point) >= 0.5:
        total_error += 1000 * np.linalg.norm(final_pos - destination_point)
    return total_error

def objective(trial):
    # steering PID search space
    skp = trial.suggest_float("steering_kp", 0.4, 1.0)
    ski = trial.suggest_float("steering_ki", 0.0, 0.3)
    skd = trial.suggest_float("steering_kd", 0.0, 0.3)
    # forward PID search space
    fkp = trial.suggest_float("forward_kp", 0.4, 1.0)
    fki = trial.suggest_float("forward_ki", 0.0, 0.3)
    fkd = trial.suggest_float("forward_kd", 0.01, 0.3)

    return run_simulation(skp, ski, skd, fkp, fki, fkd)

if __name__ == "__main__":
    sampler = optuna.samplers.TPESampler(n_startup_trials=20)
    pruner = optuna.pruners.MedianPruner(n_startup_trials=20, n_warmup_steps=8)
    study = optuna.create_study(
        direction="minimize",
        sampler=sampler,
        pruner=pruner,
        study_name="airsim_pid_tuning"
    )
    study.optimize(objective, n_trials=200, timeout=1*30*60)

    print(">> Best PID params:", study.best_params)
    with open("best_pid_params.json", "w") as fp:
