...
# Mask NaN values to keep them uncolored
masked_combined_risk_grid = ma.masked_invalid(combined_risk_grid)
...
# Plot the combined average risk grid
fig, ax = plt.subplots(figsize=(8, 6))
c = ax.pcolormesh(X, Y, masked_combined_risk_grid.T, shading='auto', cmap=cmap, vmin=0, vmax=1)

# Add a color bar to indicate the risk value (0 for ground, 1 for obstacle)
fig.colorbar(c, ax=ax, label='Average Risk Score (0-1)')
