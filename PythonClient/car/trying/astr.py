...
if __name__ == "__main__":
    ...
    # Optional: Set an initial control command before starting.
    lidar_test.client.setCarControls(airsim.CarControls(throttle=0.03, steering=0), 'CPHusky')

    # Save the starting position (set on the first loop iteration).
    ...
    try:
        while True:
            ...
            if point_cloud_data is not None:
                ...
                # On the first iteration, record the start position.
                if initial_position is None:
                    initial_position = np.array([vehicle_x, vehicle_y])
                ...
                current_pos = np.array([vehicle_x, vehicle_y])
                distance_from_start = np.linalg.norm(current_pos - initial_position)
                if distance_from_start < 2.0:
                    # print("Vehicle is within 2m of the start position. Slowing down and pausing to let it think...")
                    lidar_test.client.setCarControls(airsim.CarControls(throttle=0.0299, steering=0), lidar_test.vehicleName)
