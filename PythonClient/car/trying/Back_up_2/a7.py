   ...
   # Grid setup:
    ...
    try:
        while True:
            ...
            if valid.size:
                safe_mask = (~np.isnan(risk_grid)) & (risk_grid <= 3.0)
                valid = np.argwhere(safe_mask)
                if valid.size == 0:
                    # fallback if there are no truly safe cells
                    valid = np.argwhere(~np.isnan(risk_grid))

                centers = np.column_stack((x_mid[valid[:,0]], y_mid[valid[:,1]]))
                idx = valid[np.argmin(np.linalg.norm(centers - destination_point, axis=1))]
                temp_dest = centers[np.argmin(np.linalg.norm(centers - destination_point, axis=1))]

                # get only the XY coordinate of the vehicle
            veh_xy   = np.array([ vehicle_x, vehicle_y ])
            # make a straight‐line N‑step trajectory from vehicle → temp_dest
            ref_traj = np.linspace(veh_xy, temp_dest, N+1)[1:]
            ...
            ctr.throttle = float(np.clip(u_cmd[0]/nmpc.V_max, 0, 1))
            ...
            ax.scatter(veh_xy[1], veh_xy[0], c='green', s=50, label='Vehicle')
            ...
            ax.scatter(veh_xy[1], veh_xy[0], c='blue', s=50, label='Start')
